...
import { Log } from "@log";
import type { GuildPrefix } from "@type/constants/handler";
...
export class MessageCommand {
  ...
  private async execute(message: Message, client: EmiliaClient): Promise<undefined | Log> {
    ...
    if (!argsShift) return new Log({ text: "По неизвестным причинам argsShift == undefined", type: 2, categories: ["global", "event", "command"], tags: ["handler", "command"], code: ErrorCode.UNKNOWN_ERROR });

    const commandName = argsShift.toLowerCase();

    if (!commandName) return new Log({ text: "Произошла ошибка. CommandName = undefined", type: 2, categories: ["global", "events"], tags: ["handler", "command"], code: ErrorCode.INVALID_DATA });

    const command = client.command.get(commandName);

    if (!command) return new Log({ text: `${message.member?.user?.username ?? "[Ошибка]"} попытался(ась) заюзать ${commandName || pref} в ${message.guild?.name ?? "[Ошибка]"}`, type: 2, event: true, categories: ["global", "events"], tags: ["handler", "command"], code: ErrorCode.COMMAND_NOT_FOUND });
