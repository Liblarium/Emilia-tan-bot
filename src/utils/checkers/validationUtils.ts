import { PrimitiveType } from "@constants/enum/ValidationEnums";
import type {
  ArrayTypeCheck,
  ObjectTypeCheck,
  TypeCheck,
  ...
  ValidationOptions,
  ValidationResult,
} from "@type/utils/ValidationTypes";
...
export async function validateObject<T extends object>(
  ...
  for (const [key, check] of Object.entries(checks)) {
    ...
    // Добавляем поддержку функций-валидаторов
    if (typeof check === "function") {
    ...
    if (typeof check === "string" && !validatePrimitive(propertyValue, check)) {
      errors.push(
        createError(
          propertyPath,
          check,
