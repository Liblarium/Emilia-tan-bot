...
export async function validateObject<T extends object>(
  ...
  for (const [key, check] of Object.entries(checks)) {
    ...
    if (
      ...
      typeof check !== "object" &&
      !validatePrimitive(propertyValue, check)
    ) {
      errors.push(
        createError(
          propertyPath,
...
export async function validateSimpleTypes(
  ...
  if (
    ...
    Object.values(PrimitiveType).includes(setType<PrimitiveType>(typeCheck)) &&
    !validatePrimitive(value, setType<PrimitiveType>(typeCheck))
  ) {
