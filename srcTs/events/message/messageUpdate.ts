...
export default class MessageUpdate extends BaseEvent {
  ...
  async execute(oldMessage: MessageOrPartialMessage, newMessage: MessageOrPartialMessage, client: EmiliaClient) {
    if (!oldMessage.guild || !newMessage.guild || oldMessage.channel.isDMBased() || newMessage.channel.isDMBased() || logFilterCheck(oldMessage.channel) || logFilterCheck(newMessage.channel) || oldMessage.content === newMessage.content) return;
