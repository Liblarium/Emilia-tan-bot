import { EntityTarget, FindManyOptions, FindOptionsWhere, Repository, SaveOptions, UpdateResult } from "typeorm";
import { QueryDeepPartialEntity } from "typeorm/query-builder/QueryPartialEntity";
import { SchemaNames, schemasMap, SchemaTable } from "./schemas";
...
export class Database {
   ...
   * @param {T} name
   * @returns {Repositories[T] | undefined}
   */
   ...
   * @param {T} name имя схемы
   * @param {object} [options] дополнительные настройки для поиска.
   * @returns {Promise<{ res: SchemaTable[T][] | []; error: null | string }>}
   */
   ...
   * @param {T} name имя схемы
   * @param {FindOptionsWhere<SchemaTable[T]> | FindOptionsWhere<SchemaTable[T]>[]} filter фильтр для поиска указанных данных
   * @returns {Promise<{ res: SchemaTable[T][] | []; error: null | string }>}
   */
   ...
   * @param {T} name имя схемы
   * @param {FindOptionsWhere<SchemaTable[T]> | FindOptionsWhere<SchemaTable[T]>[]} filter фильтр для поиска указанных данных
   * @returns {Promise<{ res: SchemaTable[T] | null; error: null | string }>}
   */
   ...
   * @param {T} name имя схемы
   * @param {FindManyOptions<SchemaTable[T]>} [options] дополнительные 2 аргумента для поиска `skip` и `take`
   * @returns {Promise<{ res: SchemaTable[T][] | []; count: number; error: null | string }>}
   */
   ...
   * Ах да. `count: -1` будет в том случае, если вы указали несуществующую схему в названии или забыли её добавить к остальным в тип и массив (не тут)
   * @param {T} name имя схемы
   * @param {FindOptionsWhere<SchemaTable[T]> | FindOptionsWhere<SchemaTable[T]>[]} filter фильтр для поиска
   * @returns {Promise<{ res: SchemaTable[T][] | []; count: number; error: null | string }>}
   */
   ...
   * @param {any[]} [parameters] дополнительные парамерты для SQL запроса (в массиве)
   * @returns {Promise<{ res: any | null; error: null | string }>}
   */
  async query<T extends SchemaNames>(name: T, query: string, parameters?: any[]): Promise<{ res: any | null; error: null | string }> {
    const model = this.action(name);

    if (!model) return { res: null, error: missModelError };

    const res = await model.query(query, parameters).catch((e) => {
      console.error(e);
      return { res: null, error: e.message };
    });

    return res.error != null ? res : { res, error: null };
  }

  /**
   * Метод для создания записи в Базе Данных. Используется create() и save()
   * @param {T} name имя схемы
   * @param {SchemaTable[T]} document объект с данными для создания таблицы
   * @param {SaveOptions} [saveOptions] дополнительные опции для save
   * @returns {Promise<{ res: SchemaTable[T] | null; error: null | string }>}
   */
   ...
   * @param {T} options.name имя схемы
   * @param {FindOptionsWhere<SchemaTable[T]> | FindOptionsWhere<SchemaTable[T]>[]} options.filter что ищем
   * @param {SchemaTable[T]} options.document данные для создания, Если нет такой записи
   * @param {SaveOptions} [options.saveOptions] дополнительные опции для сохранения
   * @returns {Promise<{ res: SchemaTable[T] | null; created: boolean | null; error: null | string }>}
   */
   ...
   * @param {T} name имя схемы
   * @param {FindOptionsWhere<SchemaTable[T]>} filter какие данные нужно обновить
   * @param {QueryDeepPartialEntity<SchemaTable[T]>} update данные на изменения
   * @returns {Promise<{ res: UpdateResult | null; error: null | string }>}
   */
  async update<T extends SchemaNames>(name: T, filter: FindOptionsWhere<SchemaTable[T]>, update: QueryDeepPartialEntity<SchemaTable[T]>): Promise<{ res: UpdateResult | null; error: null | string }> {
    ...
    const upd = await model.update(filter, update).catch((e) => {
      console.error(e);
      return { res: null, error: e.message };
    });
   ...
   * @param {object} options параметры
   * @param {T} options.name имя таблицы
   * @param {FindOptionsWhere<SchemaTable[T]>} options.filter поиск по указаным данным
   * @param {SchemaTable[T]} options.document данные для создания, если нет указанных данных
   * @param {QueryDeepPartialEntity<SchemaTable[T]>} options.update то, что нужно обновить, если оно есть
   * @param {SaveOptions} [options.saveOptions] параметры для сохранения, если нет записи
   * @returns {Promise<{ res: SchemaTable[T] | UpdateResult | null; created: boolean | null; error: null | string }>}
   */
   ...
   * @param {T} name Имя схемы. С малой буквы
   * @returns {{ res: Repositories[T] | null; error: null | string }}
   */
