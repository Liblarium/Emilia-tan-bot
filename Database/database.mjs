...
export default class ConnectionInfo {
    constructor(
        ...
        if (currentConnection === undefined) {
            ...
            console.log(`[${time()}][Система | БД]: ---> Starting Database Connection`);
            new orm.DataSource(config).initialize() //createConnection
                .then(conn => {
                    this.connection = conn;
                    conn.synchronize().then(res => {
                        currentConnection = this;
                        console.log(`[${time()}][Система | БД]: ---> Database Connected Successfully`);
                        console.log(`[${time()}][Система | БД]: ConnectionComplete`);
