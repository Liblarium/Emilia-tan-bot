...
exports.ConnectionInfo = class  {
    constructor(
        ...
        if (currentConnection === undefined) {
            ...
            console.log(`[${time()}][Система | ${dbType}]: ---> Starting Database Connection`);
            new orm.DataSource(config).initialize() //createConnection
                .then(conn => {
                    this.connection = conn;
                    conn.synchronize().then(res => {
                        currentConnection = this;
                        console.log(`[${time()}][Система | ${dbType}]: ---> Database Connected Successfully`);
                        console.log(`[${time()}][Система | ${dbType}]: ConnectionComplete`);
                        return currentConnection;
                    });
                })
                .catch(err => {
                    console.error(`[${time()}][ERROR | ${dbType}]:`, err);
                    throw err;
                });
        }
    ...
    fetchData(fieldName, fieldValue, repoName, callback) {
        ...
        repo.findOne({ where: { [fieldName]: fieldValue } })
            ...
            .catch(err => {
                console.error(`[${time()}][ERROR | БД]:`, err);
    ...
    fetchByIds(ids, repoName, callback) {
        ...
        repo.findBy({id: idRef}) //findByIds
            .then(res => {
                if (res.length <= 0) return callback(undefined);
                return callback(res);
            })
